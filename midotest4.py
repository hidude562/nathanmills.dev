...
def getAImidFromSong(songName):
    ...
    def getChannelStrCharAlt(channel):
        channel = int(channel)
        match channel:
            case 0:
                out_instrument = "%"
        ...
        for msg in track:
            if str(type(msg)) == "<class 'mido.messages.messages.Message'>":
                if msg.type == "note_on":
                    ...
                    for i in range(len(notesOn) - 1, -1, -1):  # Start from the last element and go backwards
                        if notesOn[i].note == msg.note and msg.time == notesOn[i].time and getChannelStrCharAlt(notesOn[i].channel) == getChannelStrCharAlt(msg.channel):
                            noteThere = True
                            break
...
midi_dir = "/home/nathan/Desktop/NNData/Midi stuff/MIDI/" #@param {type:"string"}
...
lenMidiFiles = len(midi_files)

import json

baseSongPath = "/home/nathan/Desktop/NNData/Midi stuff/MIDI/"


def split_string(s):
    # Split the string every 2500 characters or at the next "|"
    splits = re.findall('.{1,5000}(?:\||$)', s, re.DOTALL)
    return splits
...
for songPath in midi_files:
    try:
        songSizeKB = float(os.path.getsize(songPath)) / 1024.0
        #print(songSizeKB)
        if(songSizeKB > 10.0 and songSizeKB < 50.0):
        ...
        print(e)
    fileIndex += 1
    print(fileIndex / lenMidiFiles * 100)
    if (fileIndex % 1000 == 0):
        with open("/home/nathan/.config/JetBrains/PyCharmCE2023.3/scratches/data4.0.txt", "w") as dataFile:
            print("WRITING")
            dataFile.write(allData)



with open("data4.0.txt", "w") as dataFile:
    print("WRITING")
    dataFile.write(allData)
